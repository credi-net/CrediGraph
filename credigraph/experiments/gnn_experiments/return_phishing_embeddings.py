...
parser = argparse.ArgumentParser(
    description='Return Phishing embeddings.',
...
def run_weak_supervision_forward(
    ...
    for dataset_name, path in phishing_dict.items():
        ...
        preds = all_preds_embeddings[phishing_indices]
        ...
        save_path = weight_directory / f'{dataset_name}_rni_embeddings.pkl'
        save_path.parent.mkdir(parents=True, exist_ok=True)

        with open(save_path, 'wb') as f:
            pickle.dump(domain_embeddings, f)

        logging.info(f'Saved {dataset_name} domain embeddings to {save_path}')
